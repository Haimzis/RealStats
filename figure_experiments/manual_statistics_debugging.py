...
def preprocess_and_plot():
    ...
    for dataset in tqdm(dataset_types, desc="Datasets", unit="dataset"):
        ...
        real_histograms['PatchProcessing_wavelet=ManifoldBias_level=0_patch_size=512_seed=42'] = np.array([np.array([v]) for v in real_criteria])
        fake_histograms['PatchProcessing_wavelet=ManifoldBias_level=0_patch_size=512_seed=42'] = np.array([np.array([v]) for v in fake_criteria])
